...
## LinkedIn kit 0.3.0: `connect` (2026-09-12, Sam's ask)
Sam asked for a way to connect with a profile and remove the connection. `remove_connection` already existed (profile URL as `page`, destructive). Added `connect`, same `id` + `page` shape, which lifts the version-1 rule that no tool sends on the user's behalf: a connection request is the one outward action the kit now takes, gated like every destructive tool (kit switch + `confirm: true` per call), and that gate is the built per-call approval surface, so the "deferred until that exists" condition is met. Messages still never send.
- **How it sends: LinkedIn's own invitation request, not the page's Connect button** (Sam's choice b of a/b/c). The DOM route was built first and failed live: LinkedIn's Connect button ignores presses coming from the extension's sandbox (isolated world) in every form tried (`click()`, MouseEvent, pointer sequence), while the identical press from the page's own scripts opens the dialog in 0.4s. Rejected: a per-tool "run in the page's world" flag (leaks a Chrome detail into the kit manifest, drops the sandbox) and running all catalog tools in the page world. The kit already reads the inbox through the same internal API, so `connect` posts to `voyagerRelationshipsDashMemberRelationships?action=verifyQuotaAndCreateV2` with the person's profile URN (read from the top card's Message link) and an optional `customMessage`. Verified live 2026-09-12: one invitation sent to a profile Sam picked, LinkedIn answered with an invitation URN, and the Sent page listed it at the top ("Sent today, Withdraw").
- Its own rate-limit category `invites` (10/day, editable on the kit's dashboard) rather than `actions`: LinkedIn's most-watched velocity is invitations sent (about 100/week), so it gets its own dial.
- Optional `note` (300 chars). Free accounts get a few custom notes a month (Sam's was out on 2026-09-12); when LinkedIn refuses one the tool sends NOTHING and reports it, so the caller decides whether to resend without a note. The note path is unverified for that reason.
- Reports a reason instead of sending when the page shows `pending`, `connected`, or `they_invited_you` (points to `accept_invitation`). Gotcha: the profile page lags a few seconds behind a fresh send (it still offered Connect right after; showed "Pending, click to withdraw" on the next load), so the state check is a courtesy, not the guard; the guard is LinkedIn's own answer.
- Withdraw sent invitation stays out of the kit (it lives under My network > Manage > Sent), noted in SKILL.md.
- Ships to catalog users only with the next extension version through store review (v0.1.0 is in review as of 2026-09-11); locally a reload of the unpacked extension picks it up.

## remove_connection: the confirmation matcher and accented slugs (2026-09-12)

An agent run reported `linkedin.remove_connection` dying at the confirm step ("confirmation dialog did not appear or has no Remove button") after the More menu and the "Remove connection" item both worked. Verified in Arc on a real connection (opened the confirmation, cancelled it): LinkedIn's confirmation is a native `<dialog open>` with no `role` and no `aria-modal`, heading "Remove Connection", buttons "Cancel" and "Remove connection". The tool looked only for `[role=dialog]` and a button reading exactly "Remove", so it missed both the container and the label. Fix: look in every visible dialog-like container (`dialog`, `[role=dialog]`, `[role=alertdialog]`, `.artdeco-modal`), match the button loosely (`remove`, `remove connection`, `yes, remove`), poll up to four seconds, and on failure return the headings and button labels of what was on screen instead of a bare error. `dry_run` now opens the confirmation, reports the button it would press and the container it found, then presses Cancel; the manifest description and SKILL.md say so.

Second bug found by the dry run: the "is this tab on the right profile" check compared the URL's percent-encoded slug (`la%C3%AFla-…`) to the decoded id (`laïla-…`) and refused. Both `connect` and `remove_connection` now decode before comparing. The rebuilt catalog only takes effect after the unpacked extension is reloaded; the live removal (whether the confirm button honours a press from the extension's sandbox the way the menu item did, unlike the Connect button) is still to be proven on a connection Sam picks.
